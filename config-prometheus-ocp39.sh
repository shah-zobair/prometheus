...
########################################################
# This script will prepare all the deployment scripts for deploying prometheus components.

# Make sure you have pushed following images to Internal OpenShift registry and under openshift repositry.
...
for DC in `oc get dc | grep "^router" | cut -f1 -d " "`;do
...
REGISTRY_IP=`oc get svc -n default | grep docker-registry | awk {'print $3'}`
...
#    sed -i "/ROUTER-URL/a \ \ \ \ \ \ \ \ \ \ \  ${URL}" prometheus34-deployment.yml
    sed -i "/ROUTER-URL/a \ \ \ \ \ \ \ \ \ \ \  ${URL}" prometheus39-cm.yml
    fi
...
sed -i s/ROUTER-URL/$ROUTER_URL/g prometheus39-cm.yml
...
oc patch namespace openshift-metrics -p '{"metadata": {"annotations": {"openshift.io/node-selector": ""}}}'

oadm policy add-scc-to-user anyuid -z default -n openshift-metrics
...
oadm policy add-cluster-role-to-user cluster-admin system:serviceaccount:openshift-metrics:default
oadm policy add-cluster-role-to-user cluster-admin system:serviceaccount:openshift-metrics:prometheus
 
#oc create -f prometheus-is.yml -f prometheus34-deployment.yml -f grafana-deployment.yml -n openshift-metrics
oc create -f grafana-deployment.yml -n openshift-metrics

#oc new-app prometheus -n openshift-metrics
sleep 5
oc delete cm prometheus -n openshift-metrics
oc create -f prometheus39-cm.yml -n openshift-metrics
oc new-app grafana -n openshift-metrics

for file in `ls haproxy-*-*`; do
oc create -f $file -n openshift-metrics
done
oc create -f haproxy-svc.yml -n openshift-metrics

#oc create -f node-exporter-deployment.yml -n openshift-metrics
oc create -f blackbox-exporter-deployment.yaml -f blackbox-svc.yml -n openshift-metrics
oc create -f kube-state-metrics-service.yaml -f kube-state-metrics-deployment.yaml -n openshift-metrics
